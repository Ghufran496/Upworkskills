...
- **≤13 → SKIP** (tell Saadullah why in one line; don't write it)

Always state the score breakdown and verdict before the proposal. If Saadullah says write it anyway, write it.
...
**Priority order (hard rule):** pick proof from the profile's **⭐⭐ FLAGSHIP** tier FIRST (Saad's proper full systems — use even on a 1–2% overlap), then **⭐ TOP PRIORITY**, then **Secondary**. Only paste working links; **describe-only** projects (HelloDrew, Metanoa, etc.) are cited by name with NO link. **NEVER use anything in the profile's ⛔ RETIRED list (currently Krigen AI).** Then map to the closest item:
...
Hand this chosen item to the writing layer. Saadullah claims the full skill set, but the PROOF must be real or a clearly-adjacent project — never invent a metric or client.
...
## Batch mode
If Saadullah pastes several jobs, score them all first, rank by score, and only draft the ones ≥14 — highest first. Report the skip list with one-line reasons.
...
- Mapping: 17–20 (≈9–10/10) = strong, apply; 14–16 (7–8/10) = decent; ≤13 (≤6/10) = weak. The daemon still forwards ≥4/10 so Saadullah can eyeball borderline ones via the Telegram buttons.
